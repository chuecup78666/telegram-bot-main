...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 管理員豁免 (僅針對 User，頻道身分無法判斷是否為管理員，通常視為普通成員處理)
    if user:
        try:
        ...
        try:
            ...
            if v_count >= config.max_violations:
                try: 
                    ...
                    config.add_log("WARN", f"[{msg.chat.title}] 技術禁言失敗")
